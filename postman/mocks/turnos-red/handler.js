/**
 * TurnosRed Digital Twin Mock
 * Mirrors real behavior from:
 *   src/routes/turnoRoutes.ts + src/routes/medicoRoutes.ts
 *   src/controllers/turnoController.ts + src/controllers/medicoController.ts
 *   src/services/turnoService.ts + src/services/medicoService.ts
 *   src/schemas/turnoSchema.ts + src/schemas/medicoSchema.ts
 *   src/middleware/errorHandler.ts + src/middleware/apiError.ts
 *
 * State keys (pm.state):
 *   turnos-red:turnos   → Turno[]
 *   turnos-red:medicos  → Medico[]
 *   turnos-red:seeded   → boolean
 */
...
// ─── Validation (mirrors turnoSchema.ts / medicoSchema.ts) ──────────────────
...
// ─── Normalization (mirrors turnoService.ts / medicoService.ts) ──────────────
...
// ─── Seed data (mirrors data/turnos.json — only the valid record survives normalization) ──

const SEED_TURNOS = [
  // id "102" → valid after normalization (especialidad from seed is "PEDIATRÍA" which won't
  // match schema enum, but the real app loads from file before schema validation runs;
  // we seed with already-normalized values matching what the real service would store)
...
module.exports = async function handler(req, res) {
  ...
  if (method === 'PUT' && turnoUpdateParams) {
    ...
    if (!actualizado)
      return apiError(res, 400, 'Datos del turno inválidos', 'VALIDATION_ERROR');
